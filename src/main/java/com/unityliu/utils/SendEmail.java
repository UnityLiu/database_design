...
public class SendEmail {
    ...
    private final String myEmailAccount = "unityliu@qq.com";//发送的邮箱
    private final String myEmailPassword = "zqpgburrvztydicb";
    //这个是密码（对于qq来说是授权码，下面会解释为什么不是密码）

    // 收件人邮箱（替换为自己知道的有效邮箱）
    private String receiveMailAccount = "unityliu@163.com" ;
    ...
    public  MimeMessage createMessage(Session session, String sendMail, String receiveMail,String info) throws Exception {
        ...
        message.setFrom(new InternetAddress(sendMail, "学习测试", "UTF-8"));

        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "", "UTF-8"));

        // 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject("Unityliu的验证码", "UTF-8");
